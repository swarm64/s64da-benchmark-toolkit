...
class Netdata:
    ...
    def _get_data(self, timerange, resolution):
        ...
        for chart, dimensions in self.charts.items():
            ...
            if status_code != 200:
                LOG.warning(
                    f"Netdata response not 200, but {status_code} "
                    f"for chart {chart}."
                )
    ...
    def write_stats(self, df, output):
        if len(df["stream_id"].unique()) == 1:
            ...
            LOG.info(
                "Running more than one stream. Netdata stats are written "
    ...
    def is_netdata_set_and_running(self):
        try:
            ...
            if status_code != 200:
                LOG.warning(
                    f"Netdata url response ({self.url}) does not return 200, but {status_code}"
                )
                LOG.warning(response.text)
                return False
            ...
            LOG.info(
                f"Please make sure Netdata is running, or check if the URL provided is correct"
            )
