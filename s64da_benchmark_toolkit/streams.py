...
class Streams:
    ...
    def get_stream_sequence(self, stream_id):
        ...
        with open(streams_path, "r") as streams_file:
            try:
                ...
                raise ValueError(
                    f"Stream file {streams_path} does not contain stream id {stream_id}"
                )
    ...
    def _make_run_args(self, reporting_queue):
        if self.num_streams == 0:
            ...
            return tuple(
                (reporting_queue, stream)
                for stream in range(
                    self.stream_offset, self.num_streams + self.stream_offset
                )
            )
    ...
    def _run_query(self, stream_id, query_id):
        ...
        query_sql = Streams.apply_sql_modifications(
            query_sql, (("revenue0", f"revenue{stream_id}"),)
        )
        timeout = self.config.get("timeout", 0)
        return self.db.run_query(
            query_sql, timeout, self.explain_analyze, self.use_server_side_cursors
        )
            ...
            if query_id in self.config.get("ignore", []):
                ...
                reporting_queue.put(
                    QueryMetric(
                        ...
                        + Streams.parse_timeout(self.config.get("timeout", 0)),
                        status="IGNORED",
                        result=None,
                        plan=None,
                    )
    ...
    def parse_timeout(timeout):
        valid_units = {"ms": 1, "s": 1000, "min": 60000, "h": 3.6e6, "d": 8.64e7}
