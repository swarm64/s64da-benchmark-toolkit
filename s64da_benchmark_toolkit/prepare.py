...
class PrepareBenchmarkFactory:
    ...
    def __init__(self, args, benchmark):
        schemas_dir = os.path.join(
            s64_benchmark_toolkit_root_dir, benchmark.base_dir, "schemas"
        )
        self.args = args
    ...
    def supports_cluster(self):
        ...
        if self.num_partitions:
            print(
                "Swarm64 DA CLUSTER not supported for partitioned schemas at the moment. Skipping"
            )
            return False
        ...
        if "native" in self.schema_dir:
            print(
                "Swarm64 DA CLUSTER not supported for S64 DA with Native Tables. Skipping"
            )
            return False
    ...
    def _run_tasks_parallel(self, tasks, executor_class=ThreadPoolExecutor):
        ...
        if self.args.max_jobs == 1:
            for task in tasks:
                runnable = get_runnable_task(task)
                result = (
                    runnable[0](*runnable[1:]) if len(runnable) > 1 else runnable[0]()
                )
    ...
    def _check_diskspace(self, diskpace_check_dir):
        ...
        print(
            f"Checking available diskpace in {diskpace_check_dir} with assumptions:\n"
        ...
        for table_group in PrepareBenchmarkFactory.TABLES:
            ...
            if PrepareBenchmarkFactory.PYTHON_LOADER:
                self._run_tasks_parallel(
                    ingest_tasks, executor_class=ProcessPoolExecutor
                )
            else:
    ...
    def _load_license(self, conn):
        ...
        if not license_loaded:
            try:
                ...
                print(
                    f"Could not load S64 DA license file or file is invalid: {license_path}\n"
    ...
    def prepare_db(self):
        ...
        applied_schema_path = os.path.join(
            s64_benchmark_toolkit_root_dir, "applied_schema.sql"
        )

        jinja_env = Environment(loader=FileSystemLoader(self.schema_dir))
        applied_schema = jinja_env.get_template("schema.sql").render(
        ...
        with DBConn(self.args.dsn) as conn:
            ...
            common_file_path = os.path.join(
                s64_benchmark_toolkit_root_dir, "benchmarks", "common", "functions.sql"
            )
    ...
    def add_indexes(self):
        for sql_file in (
            ...
            sql_file_path = os.path.join(
                s64_benchmark_toolkit_root_dir, self.schema_dir, sql_file
            )
    ...
    def vacuum_analyze(self):
        ...
        analyze_tasks = []
        tables = (
            PrepareBenchmarkFactory.TABLES_ANALYZE or PrepareBenchmarkFactory.TABLES
        )
