...
class DB:
    ...
    def run_query(
        self, sql, timeout, auto_explain=False, use_server_side_cursors=False
    ):
        status = Status.ERROR
        query_result = None
        plan = None
        with DBConn(self.dsn, statement_timeout=timeout) as conn:
            try:
                ...
                plan = "\n".join(
                    filter(lambda line: not line.startswith("LOG:"), notice_lines)
                )
                # make it proper json
                if plan.strip() != "":
                    plan = f"[{plan}]"
    ...
    def get_explain_output(connection, sql):
        try:
            with connection.cursor() as explain_plan_cursor:
                explain_plan_cursor.execute(
                    sql.replace("-- EXPLAIN (FORMAT JSON)", "EXPLAIN (FORMAT JSON)")
                )
