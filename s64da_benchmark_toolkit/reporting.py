...
class Reporting:
    ...
    def run_report(self, reporting_queue):
        ...
        if self.df.empty == False:
            ...
            total_runtime = (
                sub_df["timestamp_stop"].max() - sub_df["timestamp_start"].min()
            )
            ...
            LOG.warning(
                "The reporting queue was found empty, which indicates that no queries were ran"
            )
    ...
    def _save_prepare_metrics(self):
        ...
        copyfile(
            self.prepare_metrics_filename,
            os.path.join(self.results_root_dir, self.prepare_metrics_filename),
        )
...
<p>Total runtime: { self.total_runtime_seconds }s</p>\n"""
                ...
                self.df.to_html(
                    buf=html_output_file,
                    escape=False,
                    formatters={"correctness_check": lambda v: v.to_html()},
                )
        ...
        for stream_id in stream_ids:
            ...
            for index, row in sub_df.iterrows():
                if row["status"] == "OK":
                    ...
                    self.df.loc[
                        index, "correctness_check"
                    ] = correctness.check_correctness(stream_id, query_id)
