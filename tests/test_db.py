...
def test_db_reset_config(mocker):
    ...
    mock_cursor.execute.assert_has_calls(
        [call("ALTER SYSTEM RESET ALL"), call("SELECT pg_reload_conf()")]
    )
...
def test_get_explain_output_json_error(mocker):
    ...
    assert (
        plan
        == f"Explain Output failed with a JSON Decode Error: Test invalid explain plan: line 1 column 256 (char 255)"
    )
