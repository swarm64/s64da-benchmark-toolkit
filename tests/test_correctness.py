...
def check_for_mismatches(cc):
    ...
    assert cc.check_for_mismatches(
        pd.DataFrame({"foobar": ["Hello"]}), pd.DataFrame({"foobar": ["World"]})
    )


def test_check_correctness_no_correctness_file(cc, mocker):

    mocked_filepath = mocker.patch(
        "s64da_benchmark_toolkit.correctness.Correctness.get_correctness_filepath"
    )
