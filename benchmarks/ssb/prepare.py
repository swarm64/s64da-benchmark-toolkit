...
class PrepareBenchmark(PrepareBenchmarkFactory):
    PrepareBenchmarkFactory.TABLES = (
        TableGroup("date", "customer", "part", "supplier", "lineorder"),
    )

    def get_ingest_tasks(self, table):
        use_chunks = (
            self.args.scale_factor > 1
            and self.args.chunks > 1
            and table in ("customer", "lineorder")
        )
        table_code = table[0]
        ...
        if use_chunks:
            tasks = [
                f"{dbgen_cmd} -S {chunk} -C {self.args.chunks} | {sed_cmd} | {psql_copy}"
