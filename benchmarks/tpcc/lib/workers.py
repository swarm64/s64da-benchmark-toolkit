...
MAX_OLTP_WORKERS = 1000

reporting_queue = Queue()
timestamps_queue = Queue()
timestamps = Array('f', [0.0] * MAX_OLTP_WORKERS)
...
def timestamp_generator(args):
    ...
    while True:
        ...
        timestamps_queue.put_nowait(current_timestamp + increment * random.sample())
...
def tpch_worker(worker_id, args):
    queries = Queries(args.dsn, worker_id, args.olap_timeout)
    while True:
...
def run_impl(args):
    ...
    try:
        for idx in range(args.oltp_workers):
            timestamps[idx] = args.start_date.timestamp()

        original_sigint_handler = signal.signal(signal.SIGINT, signal.SIG_IGN)

        with DBConn('postgresql://postgres@localhost/htap_stats') as stats_conn:
            ...
            with Pool(processes=total_workers) as pool:
                ...
                while timestamps_queue.qsize() < 100000:
                    time.sleep(1)
                ...
                monitor.display(reporting_queue, timestamps, stats_conn)
