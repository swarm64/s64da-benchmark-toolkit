...
class PrepareBenchmark(PrepareBenchmarkFactory):
    ...
    PrepareBenchmarkFactory.TABLES = (
        TableGroup("warehouse", "item", "region", "nation", "supplier"),
    )
    PrepareBenchmarkFactory.TABLES_ANALYZE = (
        TableGroup(
            "district",
            "customer",
            "history",
    ...
    def get_ingest_tasks(self, table):
        ...
        if data_dir:
            raise ValueError(
                "Cannot use data dir with htap as this doesn't work with the process executor"
            )
            ...
            if table in ["item", "region", "nation", "supplier"]:
                ...
                return [
                    (loader.load_warehouse, dsn, w_id, start_date)
