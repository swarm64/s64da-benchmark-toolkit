...
alter table catalog_returns add constraint cr_d     foreign key (cr_returned_date_sk)       references date_dim (d_date_sk);
...
alter table catalog_returns add constraint cr_pk    foreign key (cr_item_sk, cr_order_number) references catalog_sales (cs_item_sk, cs_order_number);
...
alter table store_returns add constraint sr_pk      foreign key (sr_item_sk, sr_ticket_number) references store_sales (ss_item_sk, ss_ticket_number);
...
alter table web_returns add constraint wr_pk        foreign key (wr_item_sk, wr_order_number) references web_sales (ws_item_sk, ws_order_number);
